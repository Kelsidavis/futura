...
- **Kernel core**: 380+ Linux-compatible syscalls across 155+ implementation files. Priority-aware scheduler (nice + RT), per-task MMU contexts, COW fork, file-backed mmap, ELF loader with PT_INTERP interpreter support.
- **Testing**: **2176 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
...
- **Modern APIs**: **fsopen/fsconfig/fsmount** (Linux 5.2+), **statmount/listmount** (Linux 6.8+), **memfd_secret**, **/proc/config.gz** (65 CONFIG_* entries), **debugfs/tracefs** virtual filesystems, DMI/SMBIOS sysfs, /dev/rtc0.
...
- **Storage + VFS**: 7 filesystem drivers — **ext2/3/4**, **FAT12/16/32**, **exFAT**, **overlayfs**, **FUSE** (/dev/fuse for userspace filesystems), **FuturaFS** (native), RamFS; **cgroup2** filesystem; **debugfs/tracefs**; procfs (32 root entries including /proc/config.gz, /proc/pressure/, per-pid with 30+ files), sysfs (dynamic /sys/class/net/, /sys/class/block/, /sys/class/rtc/, /sys/bus/pci/devices/, /sys/firmware/dmi/id/), devfs (/dev/null, /dev/zero, /dev/full, /dev/urandom, /dev/random, /dev/kmsg, /dev/hwrng, /dev/rtc0, /dev/mem, /dev/ptmx, /dev/pts/\*, /dev/tty, /dev/console, /dev/net/tun, /dev/watchdog, /dev/loop0-7), **FuturaFS** (21 dedicated tests), AHCI SATA driver; **loop block devices**; block device ioctls (BLKGETSIZE64, BLKSSZGET, BLKBSZGET); **software watchdog**; real I/O accounting; full xattr, inotify, fanotify, hardlink, symlink support.
- **Shell**: 119 built-in commands with **multi-line scripting** (if/then/fi, for/in/do/done, while/do/done, case/in/esac across line boundaries), pipes (|, up to 10 stages), I/O redirection (>, >>, <), && / || chains, background jobs (&), globbing, variable expansion, command substitution; ip, ifconfig, iptables, brctl, tc for networking; top, ps, free, df, dmesg for system admin; grep, sort, wc, cut, tr, tee, diff for text processing; **Ctrl+Z job suspension**, bg/fg/jobs.
