/* font.c - Simple 8x16 bitmap font for terminal
 *
 * Copyright (c) 2025 Kelsi Davis
 * Licensed under the MPL v2.0 — see LICENSE for details.
 *
 * This implements a minimal VGA-style 8x16 bitmap font with glyphs for
 * printable ASCII characters (32-126). Each glyph is 16 bytes, one byte
 * per row, with bits representing pixels (1=foreground, 0=background).
 */
...
static const uint8_t font_glyphs[][16] = {
    /* Space (32) */
...
const uint8_t *font_get_glyph(char ch) {
    /* Map character to font_glyphs index */
    if (ch < 32 || ch > 126) {
        ch = '?';  /* Use question mark for unknown chars */
    }
