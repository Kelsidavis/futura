...
- **Graphics**: Legacy window server (`services/winsrv` + `apps/winstub`) and a Wayland compositor (`src/user/compositor/futura-wayland`) with first-party clients in `src/user/clients/` (wl-term, wl-panel, wl-settings, wl-files, wl-edit, wl-sysmon, wl-wallpaper, wl-colorwheel, wl-simple).
...
├── src/user/            # Userland services, shell, compositor, clients
