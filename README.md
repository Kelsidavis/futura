...
- **Testing**: **2047 automated kernel self-tests** across 11 test groups, all passing. ext2 verified end-to-end: mount → readdir → lookup → file read. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
- **Containers**: PID namespace foundation (`unshare(CLONE_NEWPID)` creates real namespaces, `getpid()` returns namespace-local PID, fork inherits namespace).
