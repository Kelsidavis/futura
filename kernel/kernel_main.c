...
void fut_kernel_main(void) {
    ...
    if (run_async_selftests) {
        ...
        planned_tests += 16u; /* poll: file ready, eventfd not-ready, eventfd ready, POLLNVAL, select file, select pipe, pselect6 pipe, pselect6 sigmask restore, timeout-only sleep, timerfd readiness, signalfd readiness, pipe EOF, select pipe EOF, select timerfd wakeup, poll negative fd, POLLRDNORM */
        planned_tests += 345u; /* misc(345): +copy_file_range_offsets(345) */
