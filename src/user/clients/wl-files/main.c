/* wl-files - Horizon DE file browser
 *
...
/* Directory entries — each row has a name and a file-type tag. */
#define SM_MAX_PROCS 256

/* Linux dirent64.d_type values we care about. */
#define FT_REG  8
#define FT_DIR  4
#define FT_LNK 10
#define FT_FIFO 1
...
struct proc_info {
    int pid;            /* unused, kept so renderer is unchanged   */
    char state[4];      /* unused                                  */
    char name[64];      /* directory entry name                    */
    long rss_kb;        /* unused                                  */
    char value[8];      /* "DIR ", "FILE", "LINK", etc.             */
    unsigned char type; /* dirent64 d_type for color-coding         */
};
...
static const char *type_label(unsigned char t) {
    switch (t) {
    case FT_DIR:  return "DIR";
    case FT_REG:  return "FILE";
    case FT_LNK:  return "LINK";
    case FT_FIFO: return "FIFO";
    case FT_SOCK: return "SOCK";
    case FT_CHR:  return "CHR";
    case FT_BLK:  return "BLK";
    default:      return "?";
    }
}
...
/* Sort current entries: ".." first, then directories, then files.
 * Within each group, alphabetical. Selection sort — small N. */
static int entry_class(const struct proc_info *e) {
    if (e->name[0] == '.' && e->name[1] == '.' && e->name[2] == '\0') return 0;
    if (e->type == FT_DIR) return 1;
    return 2;
}

static int entry_less(const struct proc_info *a, const struct proc_info *b) {
    int ca = entry_class(a), cb = entry_class(b);
    if (ca != cb) return ca < cb;
    /* Plain strcmp wrapped to bool */
...
static void sort_entries(void) {
    for (int i = 0; i < proc_count - 1; i++) {
        ...
        for (int j = i + 1; j < proc_count; j++) {
            if (entry_less(&procs[j], &procs[m])) m = j;
        }
        if (m != i) {
            struct proc_info t = procs[i];
            procs[i] = procs[m];
            procs[m] = t;
        }
    }
...
static void refresh_procs(void) {
    ...
    long nread;
    while ((nread = sys_call3(SYS_getdents64, fd, (long)dirent_buf,
                              sizeof(dirent_buf))) > 0) {
        long pos = 0;
        while (pos < nread) {
...
static void redraw_all(struct client_state *state) {
    ...
    for (int vi = 0; vi < SM_VIS_ROWS; vi++) {
        ...
        uint32_t type_col = COL_DIM;
        if (p->type == FT_DIR) type_col = COL_HEADER_FG;
        else if (p->type == FT_LNK) type_col = COL_WARN;
        else if (p->type == FT_REG) type_col = COL_TEXT;
...
int main(void) {
    ...
    xdg_toplevel_set_title(state.toplevel, "Files");
    xdg_toplevel_set_app_id(state.toplevel, "wl-files");
