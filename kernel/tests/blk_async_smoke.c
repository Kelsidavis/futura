...
/*
 * blk_async_smoke.c - Asynchronous block device self-test
 */
...
static void fut_blk_async_selftest_thread(void *arg) {
    ...
    rc = fut_blk_open("blk:test0", FUT_BLK_READ | FUT_BLK_WRITE | FUT_BLK_ADMIN, &rw_handle);
    if (rc != 0) {
    ...
    rc = fut_blk_open("blk:test0", FUT_BLK_READ, &ro_handle);
