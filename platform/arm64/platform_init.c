    ...
    while (*fmt) {
        if (*fmt == '%') {
            ...
            if (*fmt == 'l') {
                ...
                fmt++;
                is_size_t = 1;  /* size_t is unsigned long on ARM64 */
            }
...
static void arm64_init_spawner_thread(void *arg) {
    ...
    extern int fut_stage_wl_edit_binary(void);
    extern int fut_stage_wl_sysmon_binary(void);
    extern int fut_stage_wl_settings_binary(void);
    extern int fut_stage_wl_files_binary(void);
    extern int fut_stage_wl_wallpaper_binary(void);
    ...
    if (fut_stage_wl_edit_binary() == 0)             staged++;
    if (fut_stage_wl_sysmon_binary() == 0)           staged++;
    if (fut_stage_wl_settings_binary() == 0)         staged++;
    if (fut_stage_wl_files_binary() == 0)            staged++;
    if (fut_stage_wl_wallpaper_binary() == 0)        staged++;
    if (fut_stage_rust_hello_binary() == 0)          staged++;
    if (fut_stage_rust_uname_binary() == 0)           staged++;
    if (fut_stage_rust_pwd_binary() == 0)             staged++;
    if (fut_stage_rust_ls_binary() == 0)              staged++;
    if (fut_stage_rust_mkdir_binary() == 0)           staged++;
    if (fut_stage_rust_touch_binary() == 0)           staged++;
    if (fut_stage_rust_rm_binary() == 0)              staged++;
    if (fut_stage_rust_cat_binary() == 0)             staged++;
    if (fut_stage_rust_wc_binary() == 0)              staged++;
    if (fut_stage_rust_true_binary() == 0)            staged++;
    if (fut_stage_rust_false_binary() == 0)           staged++;
    if (fut_stage_rust_env_binary() == 0)             staged++;
    if (fut_stage_rust_head_binary() == 0)            staged++;
    if (fut_stage_rust_tail_binary() == 0)            staged++;
    if (fut_stage_rust_grep_binary() == 0)            staged++;
    if (fut_stage_rust_sleep_binary() == 0)           staged++;
    if (fut_stage_rust_date_binary() == 0)            staged++;
    if (fut_stage_rust_settings_binary() == 0)        staged++;
    if (fut_stage_rust_tree_binary() == 0)            staged++;
    if (fut_stage_rust_wallpaper_binary() == 0)       staged++;
    if (fut_stage_rust_cp_binary() == 0)              staged++;
    if (fut_stage_rust_mv_binary() == 0)              staged++;
    if (fut_stage_rust_basename_binary() == 0)        staged++;
    if (fut_stage_rust_dirname_binary() == 0)         staged++;
    if (fut_stage_rust_clear_binary() == 0)           staged++;
    if (fut_stage_rust_which_binary() == 0)           staged++;
    if (fut_stage_rust_readlink_binary() == 0)        staged++;
    if (fut_stage_rust_ln_binary() == 0)              staged++;
    if (fut_stage_rust_tee_binary() == 0)             staged++;
    if (fut_stage_rust_yes_binary() == 0)             staged++;
    ...
    if (fut_stage_rust_rev_binary() == 0)             staged++;
    if (fut_stage_rust_od_binary() == 0)              staged++;
    if (fut_stage_rust_printenv_binary() == 0)        staged++;
    if (fut_stage_rust_whoami_binary() == 0)          staged++;
    if (fut_stage_rust_id_binary() == 0)              staged++;
    if (fut_stage_rust_chmod_binary() == 0)           staged++;
    if (fut_stage_rust_hostname_binary() == 0)        staged++;
    if (fut_stage_rust_arch_binary() == 0)            staged++;
    if (fut_stage_rust_kill_binary() == 0)            staged++;
    if (fut_stage_rust_rmdir_binary() == 0)           staged++;
    if (fut_stage_rust_sync_binary() == 0)            staged++;
    if (fut_stage_rust_fold_binary() == 0)            staged++;
    if (fut_stage_rust_tac_binary() == 0)             staged++;
    if (fut_stage_rust_strings_binary() == 0)         staged++;
    if (fut_stage_rust_cut_binary() == 0)             staged++;
    if (fut_stage_rust_seq_binary() == 0)             staged++;
    if (fut_stage_rust_tr_binary() == 0)              staged++;
    if (fut_stage_rust_base64_binary() == 0)          staged++;
    if (fut_stage_rust_mktemp_binary() == 0)          staged++;
    if (fut_stage_rust_uptime_binary() == 0)          staged++;
    if (fut_stage_rust_truncate_binary() == 0)        staged++;
    if (fut_stage_rust_stat_binary() == 0)            staged++;
    if (fut_stage_rust_tty_binary() == 0)             staged++;
     ...
     * builds, or first ARM64 boots before the compositor was added). */
    {
        ...
        if (fd >= 0) {
            ...
            char *wl_envp[] = {"PATH=/sbin:/bin", "HOME=/root", "TERM=xterm-256color",
            ...
            if (ret == 0) {
                ...
                char *cli_envp[] = {"PATH=/bin:/sbin", "HOME=/root", "TERM=xterm-256color",
                                    "USER=root", "HOSTNAME=futura",
                                    "WAYLAND_DISPLAY=wayland-0",
                                    "XDG_RUNTIME_DIR=/run",
                                    "TZ_OFFSET_SEC=-25200", NULL};
                ...
                if (wlt_fd >= 0) {
                    ...
                    int wlt_ret = fut_exec_elf("/bin/wl-term", wlt_argv, cli_envp);
                    if (wlt_ret != 0) {
                        fut_printf("[INIT] wl-term exec failed (rc=%d)\n", wlt_ret);
                    }
                }

                /* Also launch wl-panel (the dock/status bar) so the
                 * desktop has its taskbar.  Best-effort — if the binary
                 * isn't present or exec fails, keep going with just the
                 * compositor + wl-term. */
                int wlp_fd = fut_vfs_open("/bin/wl-panel", 0 /* O_RDONLY */, 0);
                if (wlp_fd >= 0) {
                    fut_vfs_close(wlp_fd);
                    /* Brief wait so wl-term's surface is in place
                     * before wl-panel adds its own. */
                    for (volatile int d = 0; d < 5000000; d++);
                    char *wlp_argv[] = {"/bin/wl-panel", NULL};
                    fut_printf("[INIT] Launching wl-panel\n");
                    int wlp_ret = fut_exec_elf("/bin/wl-panel", wlp_argv, cli_envp);
                    if (wlp_ret != 0) {
                        fut_printf("[INIT] wl-panel exec failed (rc=%d)\n", wlp_ret);
                    }
                }
    ...
    char *shell_envp[] = {"PATH=/bin:/sbin", "HOME=/root", "TERM=xterm-256color",
