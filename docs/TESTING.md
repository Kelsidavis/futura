...
```
[TEST] ALL TESTS PASSED (2911/2911)
[HARNESS] PASS
```

The reachable suite count tracks the `planned_tests` sum in `kernel/kernel_main.c` (currently 2911 — five misc tests are gated out: `futex_bitset_selective_wakeup` and four branches that complete without firing `fut_test_pass`).  See `docs/ARM64_STATUS.md` for the bringup notes.
