 ...
 *
 * Phase 2 (Current): Tracks alarm expiration time (per-task storage)
...
long sys_alarm(unsigned int seconds) {
    ...
    if (seconds > 0) {
        ...
        fut_printf("[ALARM] alarm(%u) set by task %llu, expires at %llu ms (previous remaining: %u s)\n",
                   seconds, task->pid, task->alarm_expires_ms, remaining_seconds);

        /* Phase 3: Timer interrupt will check task->alarm_expires_ms and deliver SIGALRM
