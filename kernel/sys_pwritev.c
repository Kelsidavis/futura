...
ssize_t sys_pwritev(int fd, const struct iovec *iov, int iovcnt, int64_t offset) {
    ...
    /* Linux's do_pwritev runs fdget FIRST and only then dispatches into
     * vfs_writev where iovcnt=0 returns 0.  pwritev(bad_fd, _, 0, off)
    ...
    for (int i = 0; i < iovcnt; i++) {
        ...
        if (kernel_iov[i].iov_len > MAX_IOVEC_SIZE) {
            fut_printf("[PWRITEV] pwritev(fd=%d, iovcnt=%d) -> EINVAL "
    ...
    /* Success path silent — same reasoning as sys_preadv. */
    (void)zero_len_count; (void)io_pattern; (void)offset; (void)total_size;
    (void)total_written; (void)completion_status; (void)iovecs_written;
