...
- **Testing**: **2015 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
...
- **Storage + VFS**: RamFS, procfs (30 root entries, per-pid with 30+ files), sysfs (dynamic /sys/class/net/), devfs (/dev/null, /dev/zero, /dev/full, /dev/urandom, /dev/random, /dev/kmsg, /dev/ptmx, /dev/pts/\*, /dev/tty, /dev/net/tun, /dev/watchdog, /dev/loop0-7), FuturaFS, AHCI SATA driver; **loop block devices** for mounting disk images; block device layer with real /proc/diskstats and /proc/partitions; **software watchdog** (WDIOC_SETTIMEOUT, auto-reboot on hang); full xattr, inotify, hardlink, symlink support.
- **Shell**: 114 built-in commands including ip rule/tunnel/addr/route/link, ifconfig, iptables, brctl, ethtool, conntrack, losetup, mkfs.futura, ping, traceroute, netstat, ss, arp, wget, nc, httpd, nslookup, dhclient for networking; top, ps, free, df, iostat, vmstat, sysctl, dmesg, lspci, lsblk, lsof, wdctl for system admin; grep with `^`/`$`/`.`/`-w`/`-c`/`-l` pattern support, tail -f follow mode; full scripting with for/while/if, pipes, redirects, globs, command substitution, job control.
