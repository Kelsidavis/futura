...
void fut_kernel_main(void) {
    ...
    if (run_async_selftests) {
        ...
        planned_tests += 162u; /* misc(162): +futex(3), clock_nanosleep(2), mremap(1), prctl(2), madvise/mlock/getcwd/maps(4), prlimit64/mincore(2), sendfile/msync(2), stat/lstat(2), preadv/pwritev(2), uname/truncate(2), access/chdir(2), mkdir/chmod(2), unlink/rmdir/rename(2) */
