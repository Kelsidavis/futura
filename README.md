...
- **Containers**: PID namespace foundation (`unshare(CLONE_NEWPID)`), **cgroup v2** memory controller (`memory.max`, `memory.current`) and CPU controller (`cpu.max`, `cpu.weight`). Foundation for Docker/container resource limits.
