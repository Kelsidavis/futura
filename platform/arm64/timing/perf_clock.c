// SPDX-License-Identifier: MPL-2.0
/*
 * perf_clock.c - ARM64 cycle counter helpers for performance harnesses
 *
 * Uses ARM64 Generic Timer counter for high-resolution timing measurements.
 */

#include <kernel/perf_clock.h>
#include <kernel/fut_timer.h>
#include <platform/arm64/regs.h>
#include <platform/platform.h>

#ifdef __aarch64__

/* Forward declaration */
extern uint32_t fut_timer_get_frequency(void);

/**
 * Read ARM64 Generic Timer counter (CNTPCT_EL0).
 * This is the physical counter used by the system.
 */
uint64_t fut_rdtsc(void) {
    uint64_t count;
    __asm__ volatile("mrs %0, cntpct_el0" : "=r"(count));
    return count;
}

/**
 * Calibrate and return cycles-per-millisecond using generic timer.
 * Cached after first call.
 */
uint64_t fut_cycles_per_ms(void) {
    static uint64_t cached = 0;
    static int calibrated = 0;

    if (__atomic_load_n(&calibrated, __ATOMIC_ACQUIRE) && cached != 0) {
        return cached;
    }

    /* Get the timer frequency (should be set during platform init) */
    uint32_t freq = fut_timer_get_frequency();
    if (freq == 0) {
        /* Fallback: assume 1GHz if frequency not available */
        freq = 1000000000;
    }

    /* Calculate cycles per millisecond: freq / 1000 */
    uint64_t cpm = freq / 1000;
    if (cpm == 0) {
        cpm = 1;
    }

    cached = cpm;
    __atomic_store_n(&calibrated, 1, __ATOMIC_RELEASE);
    return cached;
}

/**
 * Convert cycles to nanoseconds using the cached calibration.
 */
uint64_t fut_cycles_to_ns(uint64_t cycles) {
    uint64_t cpm = fut_cycles_per_ms();
    if (cpm == 0) {
        return 0;
    }
    /* cpm is cycles per millisecond, so:
     * nanoseconds = (cycles * 1_000_000) / cpm
     */
#if defined(__SIZEOF_INT128__)
    __uint128_t scaled = (__uint128_t)cycles * 1000000ull;
    return (uint64_t)(scaled / cpm);
#else
    uint64_t quot = cycles / cpm;
    uint64_t rem = cycles % cpm;
    return quot * 1000000ull + (rem * 1000000ull) / cpm;
#endif
}

/**
 * Swap two uint64_t values (quicksort helper).
 */
static void fut_perf_swap(uint64_t *a, uint64_t *b) {
    uint64_t tmp = *a;
    *a = *b;
    *b = tmp;
}

/**
 * Sort an array of uint64_t values in ascending order.
 * Uses quicksort with manual stack to avoid recursion depth.
 */
void fut_perf_sort(uint64_t *values, size_t count) {
    if (!values || count < 2) {
        return;
    }

    typedef struct {
        size_t lo;
        size_t hi;
    } range_t;

    range_t stack[64];
    size_t top = 0;
    stack[top++] = (range_t){ .lo = 0, .hi = count - 1 };

    while (top > 0) {
        range_t seg = stack[--top];
        size_t lo = seg.lo;
        size_t hi = seg.hi;
        while (lo < hi) {
            size_t i = lo;
            size_t j = hi;
            uint64_t pivot = values[lo + ((hi - lo) >> 1)];

            while (i <= j) {
                while (values[i] < pivot) {
                    ++i;
                }
                while (values[j] > pivot) {
                    if (j == 0) {
                        break;
                    }
                    --j;
                }
                if (i <= j) {
                    fut_perf_swap(&values[i], &values[j]);
                    ++i;
                    if (j == 0) {
                        break;
                    }
                    --j;
                }
            }

            if (j > lo) {
                if (top < sizeof(stack) / sizeof(stack[0])) {
                    stack[top++] = (range_t){ .lo = lo, .hi = j };
                }
            }
            lo = i;
        }
    }
}

/**
 * Compute percentile statistics (p50, p90, p99) from sorted sample buffer.
 */
void fut_perf_compute_stats(const uint64_t *values,
                            size_t count,
                            struct fut_perf_stats *out) {
    if (!values || !out || count == 0) {
        if (out) {
            out->p50 = out->p90 = out->p99 = 0;
        }
        return;
    }

    const size_t idx50 = (count - 1u) * 50u / 100u;
    const size_t idx90 = (count - 1u) * 90u / 100u;
    const size_t idx99 = (count - 1u) * 99u / 100u;

    out->p50 = values[idx50];
    out->p90 = values[idx90];
    out->p99 = values[idx99];
}

#else
#error "perf_clock.c not implemented for this architecture"
#endif
