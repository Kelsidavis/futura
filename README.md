...
### Status Snapshot — Updated Mar 28 2026

- **Kernel core**: 400 Linux-compatible syscalls (356 x86_64 + 398 ARM64) across 226 implementation files, 110K lines of kernel C. Priority-aware scheduler (nice + RT), per-task MMU contexts, COW fork with mm_lock VMA protection, file-backed mmap, ELF loader with PT_INTERP interpreter support.
- **Testing**: **2342 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
...
- **Shell**: 177+ built-in commands with **multi-line scripting** (if/then/fi, for/in/do/done, while/do/done, until/do/done, case/in/esac), pipes (|, up to 10 stages), I/O redirection (>, >>, <), && / || chains, background jobs (&), globbing, variable expansion, `$(...)` command substitution; **awk** (field splitting, /pattern/ matching, NR/NF/$N, -F), **sed** (s/pat/rep/g), **grep -r** (recursive), sort -k/-t, wc -L, cut, tr, tee, diff, rev, nl, base64, od, xxd for text processing; enhanced **test** command (-d/-r/-w/-x/-s/-L/-h/-p operators); **eval**, **let**, **getopts**, **unset**, **return**, **shift**, **command** [-v], **expr**, **read -p/-s** for scripting; **pushd/popd/dirs** for directory navigation; **cp -r**, **rm -rf**, **ls -lh**, **head -c**, file, mkfifo, cmp, install for file management; ip, ifconfig, iptables, brctl, tc for networking; top, ps, free, df, dmesg, pgrep, pkill, pidof, nice, tput for system admin; **vi** text editor, **make** build tool, **git** version control, **curl** HTTP client, **tar** archives, **less** pager, **bc** calculator, **patch**, **sha1sum**, **nsenter**, **unshare**; **Ctrl+Z job suspension**, bg/fg/jobs.
- **IPC**: Unix domain sockets (stream, datagram, seqpacket), AF_INET TCP/UDP with loopback, **AF_INET SOCK_RAW** (ICMP echo/reply for ping), System V IPC (shm, sem, msg), POSIX message queues, pipes, eventfd, signalfd, timerfd.
...
- **Horizon Desktop Environment**: Wayland compositor with gradient background, traffic-light window buttons (close/minimize/maximize) on the left side, bottom dock with window list, click-to-focus, and clock. **Ctrl+Alt+T** launches terminal windows. VT100 terminal emulator running on real PTY devices with scrollback buffer and **ANSI 16-color** support.
- **Platforms**: x86-64 is the reference build; ARM64 port with **Raspberry Pi 4/5** and Apple Silicon support. **96 Rust driver crates**: x86-64 (**66 drivers** for AMD Ryzen AM4/AM5 + Intel Gen 10+: NVMe, AHCI SATA, xHCI USB 3.x, USB HID, USB Mass Storage, RTL8111 GbE, Intel I225-V 2.5GbE, Intel I211 GbE, HD Audio, LAPIC/IOAPIC, UART 16550, i8042 PS/2, PCIe ECAM, AMD SMBus, AMD IOMMU, AMD GPIO, AMD Watchdog, AMD SPI Flash, AMD SB-TSI temp, AMD P-State/CPPC, HPET, CMOS RTC, TPM 2.0 CRB, PCI MSI-X, AMD SMN, AMD UMC, AMD CCP crypto, ACPI PM, VESA framebuffer, PCI PM, AMD Data Fabric, AMD NBIO, AMD I2C, USB CDC/ECM net, USB Hub, USB Audio, HDA Realtek codec, AMD PSP, AMD XGBE 10GbE, x86 MCE, PCIe AER/SR-IOV, ACPI EC/Thermal, Ethernet PHY, DMA pool, FB console, EDID parser, x86 TSC, x86 CPUID, AMD SEV, AMD MP2, VirtIO console, ACPI button, Intel VT-d IOMMU, Intel HWP/Speed Shift, Intel LPSS I2C/SPI, Intel PMC, Intel PCH GPIO, Intel iTCO watchdog, Intel MEI/CSME, Intel Thunderbolt/USB4, Intel Thermal/RAPL, Intel GNA, Intel SST, Intel HDMI audio), RPi (**16 drivers**: mailbox, eMMC2/SDHCI, GPIO, GENET Ethernet, HDMI display, USB xHCI, watchdog, I2C, SPI, PWM, HW RNG, DMA, PCIe, audio, thermal/DVFS, camera), Apple Silicon (AIC, UART, RTKit, ANS2, GPIO, I2C, PCIe, SPI, DART, SMC, MCA), VirtIO (blk, gpu, input, net). RPi4/5 boot entry with EL drop, DTB detection, PL011 UART, GIC-400; complete BCM2711/BCM2712 peripheral maps; SD card config.txt generation via `make rpi-image`.
...
```bash
# Driver sets: qemu (VirtIO only), amd (Ryzen AM4/AM5), intel (Gen 10+), all
make DRIVERS=qemu          # default — lightweight for QEMU testing
make DRIVERS=amd           # AMD Ryzen bare-metal drivers
make DRIVERS=intel         # Intel bare-metal drivers
make DRIVERS=all           # every driver crate

# System profiles: server (headless) or desktop (Horizon DE + Wayland)
make PROFILE=server        # no compositor, no GUI
make PROFILE=desktop       # Horizon desktop environment
```
