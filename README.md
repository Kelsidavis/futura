...
- **Containers**: All 6 Linux namespaces — **PID**, **mount**, **UTS**, **network**, **user** (UID/GID mapping), **IPC** — plus **cgroup v2** memory+CPU controllers, **pivot_root()**, **overlayfs**. Full rootless Docker workflow: `unshare(ALL) → uid_map → mount overlay → pivot_root → exec`.
