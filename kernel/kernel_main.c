...
void fut_kernel_main(void) {
    ...
    if (run_async_selftests) {
        ...
        planned_tests += 15u; /* poll: file ready, eventfd not-ready, eventfd ready, POLLNVAL, select file, select pipe, pselect6 pipe, pselect6 sigmask restore, timeout-only sleep, timerfd readiness, signalfd readiness, pipe EOF, select pipe EOF, select timerfd wakeup, poll negative fd */
        planned_tests += 343u; /* misc(343): +signalfd_epoll(342), +signalfd_poll(343) */
