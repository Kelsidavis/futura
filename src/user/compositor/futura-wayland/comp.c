...
int comp_run(struct compositor_state *comp) {
    ...
    while (comp->running) {
        ...
        comp_run_heartbeat(comp, 1);
        wl_display_flush_clients(comp->display);

        comp_run_heartbeat(comp, 2);
        ...
        comp_run_heartbeat(comp, 3);
        if (comp->seat) {
            seat_poll_input(comp->seat);
        }

        comp_run_heartbeat(comp, 4);
