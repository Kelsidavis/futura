...
//
// rust-hostname — print the system hostname.
//
//   rust-hostname     ->  futura
//
// Calls uname(2) and prints the nodename field. Picks up
// sethostname() changes that /etc/hostname-only readers would miss.
// Doesn't yet support setting the hostname (no rust-hostname <new>
// path) — sethostname requires CAP_SYS_ADMIN and Futura's cap
// system isn't fully wired through user-space yet.
...
fn panic(_info: &PanicInfo) -> ! {
    write_str(STDERR, b"[rust-hostname] panic\n");
    unsafe {
        sys_exit(1);
    }
}
...
\0";
    ...
    if r < 0 {
        write_str(STDERR, b"rust-hostname: uname() failed\n");
        return 1;
    }
