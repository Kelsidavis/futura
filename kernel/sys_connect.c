...
long sys_connect(int sockfd, const void *addr, socklen_t addrlen) {
    ...
    fut_printf("[CONNECT] sockfd=%d addr=0x%p addrlen=%u\n", sockfd, addr, addrlen);
    ...
    fut_printf("[CONNECT] Connecting to path: '%s'\n", sock_path);

    fut_socket_t *socket = get_socket_from_fd(sockfd);
    if (!socket) {
        fut_printf("[CONNECT] ERROR: socket fd %d not valid\n", sockfd);
        return -EBADF;
    }
    ...
    fut_printf("[CONNECT] Socket %u connected to '%s'\n", socket->socket_id, sock_path);
    return 0;
}
