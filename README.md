...
- **Platforms**: x86-64 is the reference build; ARM64 port with **Raspberry Pi 4/5** and Apple Silicon support. **42 Rust driver crates**: x86-64 AMD Ryzen AM4/AM5 (**12 drivers**: NVMe, AHCI SATA, xHCI USB 3.x, RTL8111 GbE, Intel I225-V 2.5GbE, HD Audio, AMD SMBus, AMD IOMMU, AMD GPIO, AMD Watchdog, HPET, CMOS RTC), RPi (**15 drivers**: mailbox, eMMC2/SDHCI, GPIO, GENET Ethernet, HDMI display, USB xHCI, watchdog, I2C, SPI, PWM, HW RNG, DMA, PCIe, audio, thermal/DVFS), Apple Silicon (AIC, UART, RTKit, ANS2, GPIO, I2C, PCIe, SPI, DART, SMC, MCA), VirtIO (blk, gpu, input, net). RPi4/5 boot entry with EL drop, DTB detection, PL011 UART, GIC-400; complete BCM2711/BCM2712 peripheral maps; SD card config.txt generation via `make rpi-image`.
