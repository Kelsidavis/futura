...
int fut_task_waitpid(int pid, int *status_out, int flags, uint64_t *child_ticks_out) {
    ...
    for (;;) {
        ...
        if (match) {
             ...
             * Use utime_ticks + stime_ticks (from timer tick classification)
             * as the authoritative CPU time, not cpu_ticks which may include
             * double-counted context-switch deltas. */
            uint64_t child_utime = 0;
            ...
            uint64_t total_child_ticks = (child_utime + child_stime) + match->child_cpu_ticks;
            ...
            parent->child_voluntary_switches += child_voluntary + match->child_voluntary_switches;
