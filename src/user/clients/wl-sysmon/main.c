/* wl-sysmon - Minimal Wayland task manager for Horizon DE
 *
 * Copyright (c) 2025 Kelsi Davis
 * Licensed under the MPL v2.0 — see LICENSE for details.
 *
 * Scans /proc for PIDs, parses /proc/<pid>/status, renders a table of
...
static int parse_status_file(const char *path, struct proc_info *out) {
    ...
    out->name[0] = '\0';
    out->state[0] = '?'; out->state[1] = '\0';
    out->rss_kb = 0;

    char *p = buf;
    while (*p) {
        if (p[0] == 'N' && str_eq_n(p, "Name:", 5)) {
            p += 5;
            while (*p == ' ' || *p == '\t') p++;
            int j = 0;
            while (*p && *p != '\n' && j < (int)sizeof(out->name) - 1)
                out->name[j++] = *p++;
            out->name[j] = '\0';
        } else if (p[0] == 'S' && str_eq_n(p, "State:", 6)) {
            p += 6;
            while (*p == ' ' || *p == '\t') p++;
            int j = 0;
            while (*p && *p != '\n' && *p != ' ' && j < (int)sizeof(out->state) - 1)
                out->state[j++] = *p++;
            out->state[j] = '\0';
        } else if (p[0] == 'V' && str_eq_n(p, "VmRSS:", 6)) {
            p += 6;
            out->rss_kb = parse_ulong(p);
        }
        while (*p && *p != '\n') p++;
        if (*p == '\n') p++;
    }
...
static void refresh_procs(void) {
    ...
    long nread;
    while ((nread = sys_call3(SYS_getdents64, fd, (long)dirent_buf, sizeof(dirent_buf))) > 0) {
        long pos = 0;
        while (pos < nread) {
            ...
            if (is_numeric(name) && proc_count < SM_MAX_PROCS) {
                char path[64];
                int pi = 0;
                const char *pfx = "/proc/";
                while (*pfx) path[pi++] = *pfx++;
                int ni = 0;
                while (name[ni]) path[pi++] = name[ni++];
                const char *sfx = "/status";
                while (*sfx) path[pi++] = *sfx++;
                path[pi] = '\0';

                struct proc_info *pp = &procs[proc_count];
                pp->pid = str_to_int(name);
                if (parse_status_file(path, pp)) {
                    total_rss_kb += pp->rss_kb;
                    proc_count++;
                }
            }
            if (reclen == 0) break;
    ...
    /* Simple selection sort by PID (small N) */
    for (int i = 0; i < proc_count - 1; i++) {
        int m = i;
        for (int j = i + 1; j < proc_count; j++) {
            if (procs[j].pid < procs[m].pid) m = j;
        }
        if (m != i) {
            struct proc_info t = procs[i]; procs[i] = procs[m]; procs[m] = t;
        }
    }
...
/* Format RSS as KB / MB */
static int format_rss(char *out, long kb) {
    if (kb >= 10240) {
        long mb = kb / 1024;
        int n = int_to_str(out, mb, 0);
        out[n++] = 'M';
        return n;
    }
    int n = int_to_str(out, kb, 0);
    out[n++] = 'K';
    return n;
}
...
static void redraw_all(struct client_state *state) {
    ...
    /* Title + summary */
    {
        ...
        /* Right-aligned: "N procs   X MB" */
        char info[64];
        int ii = 0;
        int nn = int_to_str(info + ii, proc_count, 0); ii += nn;
        const char *lbl = " procs   ";
        while (*lbl) info[ii++] = *lbl++;
        long total_mb = total_rss_kb / 1024;
        nn = int_to_str(info + ii, total_mb, 0); ii += nn;
        const char *lb2 = " MB RSS";
        while (*lb2) info[ii++] = *lb2++;
        info[ii] = '\0';
    ...
    for (int vi = 0; vi < SM_VIS_ROWS; vi++) {
        ...
        /* PID */
        int n = int_to_str(buf, p->pid, 5);
        draw_text(px, w, h, stride, x, cy, buf, n, COL_TEXT, row_bg);
        x += 7 * FONT_WIDTH;

        /* State — color-coded */
        uint32_t state_col = COL_TEXT;
        if (p->state[0] == 'R') state_col = COL_ACCENT;
        else if (p->state[0] == 'S') state_col = COL_DIM;
        else if (p->state[0] == 'Z' || p->state[0] == 'T') state_col = COL_WARN;
        draw_text(px, w, h, stride, x, cy, p->state, 1, state_col, row_bg);
        x += 4 * FONT_WIDTH;

        /* RSS */
        n = format_rss(buf, p->rss_kb);
        /* Right-align in 8 chars */
        int pad = 7 - n;
        if (pad > 0) {
            char pbuf[16];
            for (int k = 0; k < pad; k++) pbuf[k] = ' ';
            for (int k = 0; k < n; k++) pbuf[pad + k] = buf[k];
            draw_text(px, w, h, stride, x, cy, pbuf, pad + n, COL_TEXT, row_bg);
        } else {
            draw_text(px, w, h, stride, x, cy, buf, n, COL_TEXT, row_bg);
        }
        x += 9 * FONT_WIDTH;
