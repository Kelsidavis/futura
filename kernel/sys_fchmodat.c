...
static inline int fchmodat_copy_from_user(void *dst, const void *src, size_t n) {
...
long sys_fchmodat(int dirfd, const char *pathname, uint32_t mode, int flags) {
    ...
    if (fchmodat_copy_from_user(path_buf, local_pathname, sizeof(path_buf)) != 0) {
        fut_printf("[FCHMODAT] fchmodat(dirfd=%d) -> EFAULT (copy_from_user failed)\n",
                   local_dirfd);
        return -EFAULT;
    }
    if (memchr(path_buf, '\0', sizeof(path_buf)) == NULL) {
        fut_printf("[FCHMODAT] fchmodat(dirfd=%d) -> ENAMETOOLONG\n",
                   local_dirfd);
        return -ENAMETOOLONG;
    }
