...
- **Containers**: All 6 Linux namespaces (PID, mount, UTS, network, user, IPC), **cgroup v2** with all 5 controllers (memory, CPU, I/O, PID, freezer), **pivot_root()**, **overlayfs**. Full rootless Docker workflow: `unshare → uid_map → mount overlay → pivot_root → exec`.
