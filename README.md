...
- **Testing**: **2180 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
...
- **Shell**: 125 built-in commands with **multi-line scripting** (if/then/fi, for/in/do/done, while/do/done, case/in/esac across line boundaries), pipes (|, up to 10 stages), I/O redirection (>, >>, <), && / || chains, background jobs (&), globbing, variable expansion, command substitution; ip, ifconfig, iptables, brctl, tc for networking; top, ps, free, df, dmesg for system admin; grep, sort, wc, cut, tr, tee, diff for text processing; **Ctrl+Z job suspension**, bg/fg/jobs.
