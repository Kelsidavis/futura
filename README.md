...
- **Containers**: **PID namespaces** (`unshare(CLONE_NEWPID)`), **mount namespaces** (`unshare(CLONE_NEWNS)`), **cgroup v2** memory controller (`memory.max`) and CPU controller (`cpu.max`, `cpu.weight`). Three core isolation primitives for Docker/container support.
