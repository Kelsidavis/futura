...
- **Platforms**: x86-64 is the reference build; ARM64 port with **Raspberry Pi 4/5** and Apple Silicon support. **25 Rust driver crates**: RPi (mailbox, eMMC2/SDHCI, GPIO, GENET Ethernet, HDMI display, USB xHCI, watchdog, I2C, SPI), Apple Silicon (AIC, UART, RTKit, ANS2, GPIO, I2C, PCIe, SPI, DART, SMC, MCA), VirtIO (blk, gpu, input, net). RPi4/5 boot entry with EL drop, DTB detection, PL011 UART, GIC-400; complete BCM2711/BCM2712 peripheral maps; SD card config.txt generation via `make rpi-image`.
