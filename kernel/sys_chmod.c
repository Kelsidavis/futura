...
long sys_chmod(const char *pathname, uint32_t mode) {
    ...
    *p = '\0';
    ...
    /* Linux's chmod_common just stores the new mode (masked to S_IALLUGO):
     *
     *   newattrs.ia_mode = (mode & S_IALLUGO) | (inode->i_mode & ~S_IALLUGO);
     *
     * It does NOT validate sticky/setuid/setgid bits against file type.
     * The previous Futura code rejected those combinations with -EINVAL
     * (e.g. chmod("fifo", 1755) -> -EINVAL where Linux silently accepts),
     * which broke real-world tools (rsync preserving modes from a tar
     * archive, dpkg postinst scripts that chmod system FIFOs/sockets,
     * etc.).  The sister sys_fchmod entry point already accepts the
     * full S_IALLUGO range without these checks; sys_chmod was the
     * inconsistent path.  Drop the strict gates so the stored mode
     * matches what userspace requested, matching Linux ABI and the
     * existing sys_fchmod behaviour. */
    ...
    *p = '\0';
     ...
     * caller is not in the file's group.  The check applies to
     * non-directory inodes — for directories S_ISGID has different
     * semantics (it marks files created in the directory to inherit
     * the group), so Linux preserves it across chmod regardless of
     * CAP_FSETID or group membership.  The check must consider
     * supplementary groups too (in_group_p), otherwise users who own
     * a shared-project file only via a supplementary gid silently
