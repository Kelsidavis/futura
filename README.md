...
- **Testing**: **2000 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
...
- **Shell**: 112 built-in commands including ip rule/tunnel/addr/route/link, ifconfig, iptables, brctl, ethtool, conntrack, losetup, ping, traceroute, netstat, ss, arp, wget, nc, httpd, nslookup, dhclient for networking; top, ps, free, df, iostat, vmstat, sysctl, dmesg, lspci, lsblk, lsof for system admin; grep with `^`/`$`/`.`/`-w`/`-c`/`-l` pattern support, tail -f follow mode; full scripting with for/while/if, pipes, redirects, globs, command substitution, job control.
