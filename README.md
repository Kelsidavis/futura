...
- **Testing**: **2048 automated kernel self-tests** across 11 test groups, all passing. CI: GitHub Actions with x86_64 + ARM64 QEMU test runners, consistently green.
- **Containers**: All four Linux namespaces — **PID** (`CLONE_NEWPID`), **mount** (`CLONE_NEWNS`), **UTS** (`CLONE_NEWUTS`), **network** (`CLONE_NEWNET`) — plus **cgroup v2** memory+CPU controllers, **pivot_root()** for rootfs switching. Full container init sequence: `unshare → pivot_root → exec`.
...
- **Storage + VFS**: RamFS, **ext2/3/4** (read-only), **FAT12/16/32** (read-only), **exFAT** (read-only), **overlayfs** (Docker storage driver), procfs (31 root entries including /proc/pressure/, per-pid with 30+ files), sysfs (dynamic /sys/class/net/, /sys/class/block/, /sys/bus/pci/devices/), devfs (/dev/null, /dev/zero, /dev/full, /dev/urandom, /dev/random, /dev/kmsg, /dev/hwrng, /dev/ptmx, /dev/pts/\*, /dev/tty, /dev/console, /dev/net/tun, /dev/watchdog, /dev/loop0-7), **FuturaFS** (21 dedicated tests: CRUD, mkdir, stat, ftruncate, rename, lseek, flock, O_APPEND, O_TRUNC, O_EXCL, symlinks, readdir, statfs, concurrent access), AHCI SATA driver; **loop block devices** for mounting disk images; block device layer with real /proc/diskstats and /proc/partitions; **software watchdog** (WDIOC_SETTIMEOUT, auto-reboot on hang); real I/O accounting (/proc/<pid>/io); full xattr, inotify, hardlink, symlink support.
